...
interface FrameworkTemplatesModalProps {
  isOpen: boolean;
  onClose: () => void;
  onLaunchAudit?: (framework: string) => void;
}

export const FrameworkTemplatesModal: React.FC<FrameworkTemplatesModalProps> = ({
  isOpen,
  onClose,
  onLaunchAudit
}) => {
  ...
  // Fetch framework directory from backend
  useEffect(() => {
    if (!isOpen) return;
  ...
  const handleDownload = async (format: "xlsx" | "docx" | "both") => {
    try {
      ...
      if (format === "docx" || format === "both") {
        if (data.docx_url) {
          // Slight delay for dual download
          setTimeout(() => {
            const a = document.createElement("a");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/85 backdrop-blur-md animate-in fade-in duration-150">
      <div className="w-full max-w-6xl h-[92vh] bg-[#161817] border border-[#2e3430] rounded-3xl shadow-2xl flex flex-col overflow-hidden tactile-chamfer relative font-sans">
        
        {/* Top Header Bar */}
        <div className="px-6 py-4 border-b border-[#2e3430] bg-[#1a1d1b] flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-xl bg-yellow-400/15 text-yellow-400 border border-yellow-400/30">
              <Award className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base font-bold text-[#f5f1e8]">
                  Statutory Accreditation & Ranking Framework Templates
                </h2>
                <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-[#4b9b74]/15 text-[#4b9b74] border border-[#4b9b74]/30 font-semibold flex items-center gap-1">
                  ...
                  7 Sovereign Templates Integrated
                </span>
              </div>
              <p className="text-[11px] text-[#b8b2a4]">
                Official submission dossiers (.docx) and multi-sheet data capture workbooks (.xlsx) grounded in verified university RAG records.
              </p>
            </div>
          </div>
          ...
          <button
            ...
            title="Close Framework Templates Cockpit"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {/* Success Banner */}
        {downloadSuccess && (
          <div className="px-6 py-2.5 bg-[#4b9b74]/20 border-b border-[#4b9b74]/30 text-[#4b9b74] text-xs font-mono flex items-center gap-2 animate-in fade-in">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-72 sm:w-80 border-r border-[#2e3430] bg-[#131514] flex flex-col shrink-0 overflow-y-auto p-3 space-y-1.5">
            <div className="px-2 py-1 text-[10px] font-mono font-bold tracking-wider text-[#7d776b] uppercase">
          ...
          {/* Right Column: Active Framework Deep Dive & Generation */}
          <div className="flex-1 bg-[#161817] overflow-y-auto p-6 space-y-6">
            ...
            {/* Criteria Breakdown & Quantitative Sheets (2 Grids) */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              
              {/* Left: Criteria & Rubrics */}
              ...
              {/* Right: Quantitative Multi-Sheet Architecture */}
              <div className="p-4 rounded-2xl bg-[#1a1d1b] border border-[#2e3430] space-y-3">
            ...
            {/* Official Submission Standards & Verification Sign-Off Footer */}
